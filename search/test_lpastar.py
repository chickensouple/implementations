...
class Application(tk.Frame):
    ...
    def run_lpa(self):
        path_found, path, cost, nodes_expanded = self.lpastar.search()
        self.found_path_lock.acquire()
    ...
    def update(self):
        ...
        if (start_goal_changed or graph_changed) and start != None and goal != None:
            # only start new lpa thread if there isn't one or its finished
            if self.lpa_thread == None or not self.lpa_thread.isAlive():

                heuristic = partial(cost_heuristic_linf, goal=goal)
                graph = self.map_data.get_graph()

                if start_goal_changed:
                    # only create new LPA star if start or goal is changed
                    self.lpastar = LPAStar(graph, start, goal, cost_heuristic=heuristic)
                elif graph_changed:
                    # update lpastar with changed nodes
                    self.lpastar.update_graph(graph)
                    change_node_set = self.map_data.get_changed_data()
                    for node in change_node_set:
                        self.lpastar.update_node(node)
