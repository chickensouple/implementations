...
def cost_heuristic_l1(node, goal):
    h = np.linalg.norm(np.array(goal) - np.array(node), ord=1)
    return h

def cost_heuristic_l2(node, goal):
    h = np.linalg.norm(np.array(goal) - np.array(node), ord=2)
    return h

def cost_heuristic_linf(node, goal):
    h = np.linalg.norm(np.array(goal) - np.array(node), ord=np.Inf)
    return h
